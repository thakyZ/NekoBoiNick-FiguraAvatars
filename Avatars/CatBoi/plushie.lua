...
-- config
local model = models.CatBoi.Skull -- model part for plushie
...
local function myHead(bl)
  ...
  if data and data.SkullOwner and data.SkullOwner.Id then
    local unpacked_table = table.unpack(data.SkullOwner.Id) ---@type integer[]
    print(unpacked_table[1], unpacked_table[2], unpacked_table[3],
      unpacked_table[4])
    return client:intUUIDToString(table.unpack(data.SkullOwner.Id)) == myUuid
  end
  return false
end
