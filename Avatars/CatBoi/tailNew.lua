-- config
local tailConfig = {
   model = models.CatBoi.root.Body.tail,

   velocityStrength = vec(1, 1, 1), -- left right, up down, forward backward

   rotVelocityStrength = 4,
   rotVelocityLimit = 10,

   verticalVelocityMin = -5,
   verticalVelocityMax = 2,

   bounce = vec(0.07, 0.07, 0.1, 0.1),  -- how bouncy tail will be, can also be vector 4
   stiff = vec(0.08, 0.08, 0.18, 0.18), -- how stiff should tail be, can also be vector 4
   waterStiff = 0.5,                    -- how stiff should tail be underwater
   waterStrength = 0.5,                 -- how much water will affect tail

   idleSpeed = vec(0, 0.1, 0.1),        -- how fast should tail move when nothing is happening
   ...
   wagStrength = vec(20, 12, 0),        -- how much it should move
...
function events.tick()
   ...
   bodyVel = math.clamp(bodyVel * tailConfig.rotVelocityStrength * 0.2, -tailConfig.rotVelocityLimit,
      tailConfig.rotVelocityLimit)
   local wagWalkSpeed = tailConfig.walkLimit == 0 and 0 or
       math.clamp(playerVelRaw.z * tailConfig.velocityStrength.z / tailConfig.walkLimit, 0, 1)
   -- water level
   ...
   rot[1].x = rot[1].x +
       math.clamp(playerVel.y * 5 - inWater * 4, tailConfig.verticalVelocityMin,
          tailConfig.verticalVelocityMax)
   rot[1].y = rot[1].y + bodyVel * math.max(1 - math.abs(playerVelRaw.x) * 4, 0) +
       math.clamp(playerVel.x * 20, -2, 2)
   rot[1].w = rot[1].w *
       math.clamp(1 - playerVel.z - math.abs(bodyVel) * 0.02 + playerVel.y * 0.25 - inWater * 0.25, 0,
          1)
...
local function getPartRot(i, delta, time, strength)
   ...
   return r.xyz + defaultRot[i] * r.w +
       (time - tailConfig.tailOffset * i):applyFunc(math.sin) * strength
end
