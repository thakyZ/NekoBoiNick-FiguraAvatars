...
local s, this = pcall(function()
  ...
  function callbackGenerators.dualBlend(blend_in, blend_out)
    ...
    if tbin == "string" then
      ...
      if not infunc then
        error(
          "bad argument #1 to 'dualBlend' ('" .. blend_in .. "' is not a valid curve)", 2)
      end
    ...
    if tbout == "string" then
      ...
      if not outfunc then
        error(
          "bad argument #2 to 'dualBlend' ('" .. blend_in .. "' is not a valid curve)", 2)
      end
        ...
        if state.done then
          ...
          elseif (initSlope == 0) then
            ...
            while (m_abs(curX) > self.options.subdiv_prec and iter < self.options.subdiv_iters) do
              curT = A + (B - A) * 0.5
              --[[calcBezier(curT, X1, X2) - X]]
              curX = ((((1.0 - 3.0 * X2 + 3.0 * X1) * curT + (3.0 * X2 - 6.0 * X1)) * curT + (3.0 * X1)) * curT) -
                  X
              if curX > 0.0 then B = curT else A = curT end
              iter = iter + 1
            end
          ...
          --#end getTForX#
          --[[calcBezier(T, Y1, Y2)]]
          local prog = (((1.0 - 3.0 * Y2 + 3.0 * Y1) * T + (3.0 * Y2 - 6.0 * Y1)) * T + (3.0 * Y1)) *
              T
  ...
  function callbackGenerators.timeline(tl)
    ...
    if this.safe then
      ...
      if ftime ~= 0 then
        error("error in keyframe #1: timeline does not start at 0 (got " ..
          ftime .. ")")
      end
      for i, kf in ipairs(tl) do
  ...
  function animationSetters:blendCallback(value)
    ...
    if type(func) == "string" then
      ...
      if not value then
        error("bad argument #3 of '__newindex' ('" ..
          func .. "' is not a valid curve)")
      end
    end
  ...
  function animationMethods:setOnBlend(func)
    ...
    if type(func) == "string" then
      ...
      if not func then
        error("bad argument #2 of 'setOnBlend' ('" ..
          func .. "' is not a valid curve)")
      end
    end
...
if s then
  ...
  for line in e_stack:gmatch("[ \t]*([^\n]+)") do
    ...
    elseif line:match("in function 'pcall'") then
      ...
      stack_lines[#stack_lines] = stack_lines[#stack_lines]:gsub("in function %b<>",
        "in protected chunk")
      skip_next = true
    end
  end
  ...
  if FIG[1] and cmp(ver, FIG[1]) == -1 then
    extra_reason = ("\n§oYour Figura version (%s) is below the recommended minimum of %s§r"):format(
      ver, FIG[1])
  elseif FIG[2] and cmp(ver, FIG[2]) == 1 then
    extra_reason = ("\n§oYour Figura version (%s) is above the recommended maximum of %s§r"):format(
      ver, FIG[2])
  end
